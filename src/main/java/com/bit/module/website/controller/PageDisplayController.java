...
public class PageDisplayController {
    ...
    */
    ...
    public void downLoadFile(HttpServletResponse response,
        ...
        ServiceType byId = serviceTypeDao.findById(id);
        String filesDetail = byId.getFilesDetail();
        List<FileInfo> fileInfos = JSON.parseArray(filesDetail, FileInfo.class);
        if (fileInfos == null || fileInfos.size() <= 0) {
            throw new BusinessException("无文件");
        }
        try {
            //结果集转换成map
            Map<Long, FileInfo> fileInfoMap = fileInfos.stream().collect(Collectors.toMap(FileInfo::getId, Function.identity()));
